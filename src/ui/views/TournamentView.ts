...
export class TournamentView {
  ...
  public refreshRoster(): void {
    ...
    this.selectFighter.innerHTML = list.map(w => `<option value="${w.name}">${w.name} [${w.type}]</option>`).join("");
  }

  private seed(): void {
    const all = this.gm.getAllWarriors().map(w => w.name);
    if (all.length < 8) { this.status.textContent = "Need at least 8 warriors."; return; }
    const player = this.selectFighter.value || all[0];

    const res = this.cmdBus.dispatch(new SeedTournamentCommand(player, all));
    if (!res.ok) { this.status.textContent = res.error; return; }
    ...
    const res = this.cmdBus.dispatch(new PlayPlayerMatchCommand((a, b, done) => {
      this.cb.onPlayMatch(a, b, (winner) => {
        ...
        this.status.textContent = this.cmdCtx.tour!.isFinished()
          ...
          : "Round updated. Click Play to continue.";
      });
    }));

    if (!res.ok) {
      this.renderBracket();
      if (this.cmdCtx.tour!.isFinished()) this.status.textContent = `🏆 ${this.cmdCtx.tour!.getWinner()} wins!`;
    }
  }
